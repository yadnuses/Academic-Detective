...
- 46条学者档案已回填部分字段
- 10条异常规则已激活
- 批量计算完成，6个确认不端案例被正确识别（钟华57.27、张裕卿12.95、黄飞若7.28、贺海波5.67等）

v1.0 暴露问题：
- 19个确认不端案例异常指数为0（数据回填缺失）
- A005权重过高导致分数压缩（单个规则产生428分）
- 顶尖学者谢毅（院士，h_index=104）被误标为medium风险
- 审稿周期、合作者集中度等字段完全缺失
...
| 优先级 | 任务 | 预期效果 | 验收标准 |
|:---:|:---|:---|:---|
| P0 | 回填19个0分不端案例的 h_index + total_citations + num_papers_verified | 让刘翔峰、翟天临等进入检测范围 | 19个案例异常指数>0 |
| P1 | 降低A005权重 + 引入分数标准化 | 分数分布均衡，区分度提升 | 最高分<20，最低分>0的不端案例>15个 |
| P2 | 顶尖学者分层基线 | 消除谢毅等院士的误报 | 谢毅异常指数<1.0 |
...
| 案例 | 姓名 | 建议数据源 | 备注 |
|:---|:---|:---|:---|
| CASE_002 | 刘翔峰 | 知网、PubMed、湘雅二医院官网历史快照 | 临床医生，学术产出有限，h_index可能<10 |
| CASE_024 | 翟天临 | 知网、Google Scholar | 戏剧影视学，知网可查其博士论文和发表论文 |
| CASE_029 | 西安交大王某某 | 知网、WOS | 匿名案例，可用机构+领域+时间范围检索 |
| CASE_030 | 对外经贸李某某 | 知网、WOS | 匿名案例，同上 |
| CASE_037 | 钟文/月福财 | 知网、WOS | 化学领域，可用姓名+机构检索 |
| CASE_038 | 公绪合 | 知网、WOS | 化学领域 |
...
| CASE_036 | 黄冠群 | 如无法获取，h_index=NULL |
| CASE_044 | 陈岩 | 同上 |
...
谢毅（CASE_019）：中科院院士，h_index=104，年均发文66篇。
...
例如：
- 刘翔峰的h_index=0：可能是真的h_index=0（无学术影响力），也可能是数据缺失
