...
-- v3.1 reactive state machine tables
CREATE TABLE IF NOT EXISTS case_states (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    case_name   TEXT NOT NULL UNIQUE,
    phase       TEXT NOT NULL DEFAULT 'initialized',
    phase_entered_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    phase_exited_at  TIMESTAMP,
    created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS unify_records (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    case_name   TEXT NOT NULL,
    phase       TEXT NOT NULL,
    planned     TEXT,
...
CREATE INDEX IF NOT EXISTS idx_recs_case ON recommendations(case_name);
CREATE INDEX IF NOT EXISTS idx_recs_status ON recommendations(status);
CREATE INDEX IF NOT EXISTS idx_decisions_case ON decisions(case_name);
CREATE INDEX IF NOT EXISTS idx_unify_case ON unify_records(case_name);
"""
...
class InvestigationDB:
    ...
    def reset(self):
        ...
        with self._connect() as conn:
            tables = [
                ...
                "case_states", "recommendations", "decisions", "unify_records",
            ]
            for t in tables:
    ...
    def init_schema_v3(self):
        """Create v3.1 reactive state machine tables."""
        v3_sql = """
        CREATE TABLE IF NOT EXISTS case_states (
            id          INTEGER PRIMARY KEY AUTOINCREMENT,
            case_name   TEXT NOT NULL UNIQUE,
        ...
        with self._connect() as conn:
            ...
            conn.commit()
        logger.info("Schema v3.1 initialized: %s", self.db_path)
    ...
    # -------------------------------------------------------------------
    # Case state tracking (v3.1)
    # -------------------------------------------------------------------

    def set_case_phase(self, case_name: str, phase: str):
        """Update or insert case state with new phase."""
        with self._connect() as conn:
            row = conn.execute(
                "SELECT id, phase FROM case_states WHERE case_name=?",
                (case_name,),
            ).fetchone()
            ...
            if row:
                conn.execute(
                    """
                    UPDATE case_states
                    SET phase=?, phase_entered_at=?, phase_exited_at=NULL,
                        updated_at=?
                    WHERE case_name=?
                    """,
                    (phase, now, now, case_name),
                )
            else:
                conn.execute(
                    """
                    INSERT INTO case_states (case_name, phase, phase_entered_at, updated_at)
                    VALUES (?, ?, ?, ?)
                    """,
                    (case_name, phase, now, now),
                )
            conn.commit()
        logger.info("Case '%s' phase set to '%s'", case_name, phase)

    def get_case_phase(self, case_name: str) -> str:
        """Get current phase for case, or 'initialized' if not found."""
        with self._connect() as conn:
            row = conn.execute(
                "SELECT phase FROM case_states WHERE case_name=?",
                (case_name,),
            ).fetchone()
            return row["phase"] if row else "initialized"

    # -------------------------------------------------------------------
    # Recommendations (v3.1)
    # -------------------------------------------------------------------

    def add_recommendation(
        self, case_name: str, rule_id: str, tools: list, priority: int, reason: str
    ) -> int:
        """Insert a new recommendation. Returns the new row id."""
        with self._connect() as conn:
            cursor = conn.execute(
                """
                INSERT INTO recommendations (case_name, rule_id, tools, priority, reason, status)
                VALUES (?, ?, ?, ?, ?, 'pending')
                """,
                (case_name, rule_id, json.dumps(tools, ensure_ascii=False), priority, reason),
            )
            conn.commit()
            return cursor.lastrowid

    def get_pending_recommendations(self, case_name: str) -> list[dict]:
        """Return all pending recommendations for a case."""
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT * FROM recommendations
                WHERE case_name=? AND status='pending'
    ...
    def update_recommendation_status(self, rec_id: int, status: str):
        """Update recommendation status (pending/accepted/rejected/completed)."""
        with self._connect() as conn:
            now = datetime.now().isoformat()
            if status == "accepted":
                conn.execute(
                    "UPDATE recommendations SET status=?, accepted_at=? WHERE id=?",
                    (status, now, rec_id),
                )
            elif status == "completed":
                conn.execute(
                    "UPDATE recommendations SET status=?, completed_at=? WHERE id=?",
                    (status, now, rec_id),
                )
            else:
                conn.execute(
            ...
            conn.commit()
        logger.info("Recommendation %d status updated to '%s'", rec_id, status)

    # -------------------------------------------------------------------
    # Human decisions log (v3.1)
    # -------------------------------------------------------------------

    def log_decision(
        self, case_name: str, decision: str, context: str = "", made_by: str = "human"
    ):
        """Log a human or agent decision."""
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO decisions (case_name, decision, context, made_by)
                VALUES (?, ?, ?, ?)
                """,
                (case_name, decision, context, made_by),
            )
            conn.commit()
        logger.info("Decision logged for '%s': %s", case_name, decision)

    # -------------------------------------------------------------------
    # UNIFY records (v3.1)
    # -------------------------------------------------------------------

    def add_unify_record(
        self, case_name: str, phase: str, planned: str, actual: str,
        deviations: str, decisions: str
    ):
        """Insert a phase post-mortem UNIFY record."""
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO unify_records (case_name, phase, planned, actual, deviations, decisions)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (case_name, phase, planned, actual, deviations, decisions),
            )
            conn.commit()
        logger.info("UNIFY record added for '%s' phase '%s'", case_name, phase)

    def get_unify_records(self, case_name: str) -> list[dict]:
        """Return all UNIFY records for a case, newest first."""
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM unify_records WHERE case_name=? ORDER BY created_at DESC",
                (case_name,),
            ).fetchall()
            return [dict(r) for r in rows]
