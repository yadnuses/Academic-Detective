...
def build_scholar_data(config: dict, data_dir: Path) -> dict:
    ...
    # --- Data collection version tracking ---
    data_collection_round = _infer_collection_round(data_dir)
    ...
    data = {
        ...
        "_version": {
            "data_collection_round": data_collection_round,
            "builder_build_timestamp": builder_timestamp,
            "config_version": config.get("config_version", "2.0"),
            "serial": f"R{data_collection_round:03d}_{builder_timestamp[:10]}",
        },
        ...
        "peer_cohort": _build_peer_cohort(config),
    }

    return data


def _infer_collection_round(data_dir: Path) -> int:
    """
    Infer the current data collection round number from existing artifacts.
    Scans for prior scholar_data.json or checkpoint files and increments.
    """
    round_markers = []

    # Check for prior scholar_data.json
    for p in [data_dir / "scholar_data.json", data_dir.parent / "scholar_data.json"]:
        if p.exists():
            try:
                with open(p, "r", encoding="utf-8") as f:
                    prior = json.load(f)
                ver = prior.get("_version", {})
                prev_round = ver.get("data_collection_round", 0)
                if isinstance(prev_round, int) and prev_round > 0:
                    round_markers.append(prev_round)
            except Exception:
                pass

    # Check for collection checkpoint files
    for dirpath in data_dir.rglob("collection_checkpoint.json"):
        try:
            with open(dirpath, "r", encoding="utf-8") as f:
                cp = json.load(f)
            r = cp.get("collection_round", 0)
            if isinstance(r, int) and r > 0:
                round_markers.append(r)
        except Exception:
            pass

    if round_markers:
        return max(round_markers) + 1
    return 1


def _build_peer_cohort(config: dict) -> list:
    """
    Build peer cohort comparison data from config.
    Reads peer_cohort section and returns structured comparison list.
    """
    peers_raw = config.get("peer_cohort", []) or config.get("xiaohongshu", {}).get("peer_cohort", [])
    if not peers_raw:
        return []
    ...
    for peer in peers_raw:
        cohort.append({
            "name": peer.get("name", "未知"),
            "associate_year": peer.get("associate_year"),
            "full_year": peer.get("full_year"),
...
def _summarize_text_profiles(profiles: list) -> dict:
    ...
    # Aggregate paper type distribution
    paper_types = {}
    for p in valid:
        pt = p.get("paper_type_classification", {})
        t = pt.get("type", "unknown") if pt else "unknown"
    ...
    return {
        ...
        "paper_type_distribution": paper_types,
    }
...
def _summarize_style_report(report: dict | None) -> dict:
    ...
    heatmap = report.get("word_frequency_heatmap", {})
    heatmap_status = heatmap.get("status", "unknown") if heatmap else "not_generated"
    return {
        "status": "loaded",
        "document_count": report.get("document_count"),
        "red_flags": [f["signal"] for f in report.get("red_flags", [])],
        "word_frequency_heatmap": {
            "status": heatmap_status,
            "max_abs_deviation": heatmap.get("max_abs_deviation") if isinstance(heatmap, dict) else None,
            "n_baseline_docs": heatmap.get("n_scholar_baseline_docs") if isinstance(heatmap, dict) else None,
        },
    }
