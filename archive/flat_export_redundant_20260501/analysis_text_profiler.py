...
New in v2.0:
  - paper_type_classification: auto-detects academic_paper / academic_review /
    essay_commentary / non_academic with confidence levels.
...
def classify_paper_type(text: str, basic_stats: dict = None) -> dict:
    """
    Classify a document's paper type based on structural and content features.

    Returns one of:
      - 'academic_paper': 正规学术论文（有摘要、方法、参考文献等标准结构）
      - 'academic_review': 学术评论/书评/会议述评（有学术性但非标准论文）
      - 'essay_commentary': 随笔/短评/读后感（缺乏学术规范）
      - 'non_academic': 非学术（新闻、报告、其他与学术无关的内容）

    Confidence levels: high / medium / low
    """
    stats = basic_stats or compute_basic_stats(text)
    result = {"type": None, "confidence": "low", "features": {}}

    chinese_chars = stats.get("chinese_characters", 0)
    total_chars = stats.get("total_characters", 0)

    # --- Feature extraction ---

    # 1. Has abstract section (标准学术论文特征)
    has_abstract_cn = bool(re.search(r"[摘〔【][要　]", text[:5000]))
    has_abstract_en = bool(re.search(r"Abstract", text[:5000], re.IGNORECASE))
    has_keywords = bool(re.search(r"[关关键词][键词]?[词:：　]", text[:5000]))
    has_abstract = has_abstract_cn or has_abstract_en

    # 2. Has references
    ref_header_patterns = [
        r"参考[文献书目]", r"[Rr]eference[s]?", r"[Bb]ibliography",
        r"注释", r"[文注][献释]",
    ]
    has_references = False
    for pat in ref_header_patterns:
        m = re.search(pat + r"[\s:：\n]", text[-10000:], re.IGNORECASE | re.MULTILINE)
        if m:
            has_references = True
            break
    # Count reference-like lines in the last portion
    ref_lines = 0
    for line in text[-10000:].splitlines():
        line = line.strip()
        if len(line) > 10 and re.search(r"[\u4e00-\u9fff]", line[:5]) and re.search(r"(19|20)\d{2}", line):
            ref_lines += 1

    # 3. Has method/methodology section
    has_method_section = bool(re.search(
        r"(研究[方法设计路径]|方法论|[方研][法究][方设]法?[\s:：])",
        text[:10000], re.IGNORECASE
    ))

    # 4. Has chapter structure (章节)
    chapter_headers = re.findall(r"^\s*(第[一二三四五六七八九十0-9]+[章节部编]|[一二三四五六七八九十]+[、\.]\s*[\u4e00-\u9fff])", text, re.MULTILINE)
    has_chapters = len(chapter_headers) >= 3

    # 5. References count indicator
    ref_count_indicator = ref_lines

    # 6. Estimated length in pages
    est_pages = chinese_chars / 800  # rough: ~800 chars per page

    # 7. Originality markers
    orig_markers = count_originality_markers(text)
    has_originality_claims = orig_markers["total"] >= 2

    # 8. Title indicators
    first_500 = text[:500]
    has_review_title = bool(re.search(r"(评|论|读后感|观后|关于|读|Review|Commentary|Note)", first_500))

    # 9. Data availability / figures
    has_data_statement = bool(re.search(r"数据[可共]用|data availab|补充材料|supplementary", text[:5000], re.IGNORECASE))
    figure_refs = len(re.findall(r"[图F][iI][gG]?\.?\s*\d+", text))
    table_refs = len(re.findall(r"[表T][aA][bB][lL][eE]?\.?\s*\d+", text))

    features = {
        "has_abstract": has_abstract,
        "has_keywords": has_keywords,
        "has_references": has_references,
        "ref_line_count": ref_count_indicator,
        "has_method_section": has_method_section,
        "has_chapters": has_chapters,
        "est_pages": round(est_pages, 1),
        "has_originality_claims": has_originality_claims,
        "has_review_title_indicator": has_review_title,
        "has_data_statement": has_data_statement,
        "figure_ref_count": figure_refs,
        "table_ref_count": table_refs,
    ...
    if has_method_section:
        score_academic += 15
    if has_chapters and est_pages >= 3:
        score_academic += 10
    if has_references and ref_count_indicator >= 5:
        score_academic += 15
    elif has_references and ref_count_indicator >= 2:
        score_academic += 8
    if has_data_statement or figure_refs >= 3:
        score_academic += 10
    if has_originality_claims:
        score_academic += 10
    if est_pages >= 5:
        score_academic += 5

    # Review/commentary indicators
    if has_review_title:
        score_review += 20
    if has_references and ref_count_indicator <= 10:
        score_review += 5
    ...
    # Essay/commentary indicators
    if not has_references and ref_count_indicator <= 2:
        score_essay += 25
    if not has_abstract:
        score_essay += 15
    if est_pages < 3:
    ...
    result["scores"] = {
        "academic_paper": score_academic,
        "academic_review": score_review,
        "essay_commentary": score_essay,
    }

    # Decide
    max_score = max(score_academic, score_review, score_essay)
    if max_score == 0:
        result["type"] = "non_academic"
        result["confidence"] = "low"
    elif score_academic >= score_review and score_academic >= score_essay:
        if score_academic - max(score_review, score_essay) >= 20:
            confidence = "high"
        elif score_academic - max(score_review, score_essay) >= 10:
            confidence = "medium"
        else:
            confidence = "low"
        if score_academic >= 50:
            result["type"] = "academic_paper"
            result["confidence"] = confidence
        else:
            # Falls into low-score academic category
            margin = score_academic - max(score_review, score_essay)
            if margin < 10 and score_review >= score_academic and est_pages < 8:
                result["type"] = "academic_review"
                result["confidence"] = "medium" if score_review >= 40 else "low"
            else:
                result["type"] = "academic_paper"
                result["confidence"] = "low"
    elif score_review >= score_essay:
        result["type"] = "academic_review"
        result["confidence"] = "high" if score_review >= 50 else ("medium" if score_review >= 35 else "low")
    else:
        result["type"] = "essay_commentary"
        result["confidence"] = "high" if score_essay >= 40 else ("medium" if score_essay >= 25 else "low")
...
def main():
    ...
    profile = {
        ...
        "paper_type_classification": classify_paper_type(text, basic_stats),
    ...
    print(f"[SUMMARY] Source: {profile['source_file']}, Characters: {profile['basic_stats']['total_characters']}, "
          ...
          f"Paper type: {profile['paper_type_classification']['type']} "
          f"(confidence: {profile['paper_type_classification']['confidence']})")
