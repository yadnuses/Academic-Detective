...
| 5. Anomaly Detection | LLM | `domestic/data_validator.py` + `deep_evidence/data_forensics/data_integrity_checker.py` | Flagged inconsistencies with confidence scores; **数据取证子模块**: 当有论文原始数据(Excel/CSV)时，运行数据造假检测（尾数分布/小数位一致性/数值重复） |
